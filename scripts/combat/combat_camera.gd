...
var _warped_this_frame: bool = false
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		if _warped_this_frame:
			# This motion event was generated by warp_mouse — ignore it.
			_warped_this_frame = false
			return
...
func _process(_delta: float) -> void:
	...
	# Keep mouse hidden during gameplay.
	if not get_tree().paused:
		if Input.get_mouse_mode() != Input.MOUSE_MODE_HIDDEN:
			Input.set_mouse_mode(Input.MOUSE_MODE_HIDDEN)
		# Warp mouse to center each frame. The _input handler ignores the
		# warp-generated motion by checking _warped_this_frame.
		_warped_this_frame = true
		get_viewport().warp_mouse(_screen_center)
